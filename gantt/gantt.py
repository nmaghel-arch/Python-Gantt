...
class Task(_scale):
    ...
    def svg(self, prev_y=0, start=None, end=None, color=None, level=None, scale=DRAW_WITH_DAILY_SCALE, title_align_on_left=False, offset=0):
        ...
        if self.resources is not None:
            ...
            svg.add(svgwrite.text.Text("{0}".format(t), insert=(tx*mm, (y + 8.5)*mm), fill=DEFAULT_TASK_TEXT_COLOR, stroke=_font_attributes()['stroke'], stroke_width=_font_attributes()['stroke_width'], font_family=_font_attributes()['font_family'], font_size=15-5))
                ...
                if isinstance(t, Milestone):
                    if not (t.drawn_x_end_coord is None or t.drawn_y_coord is None or self.drawn_x_begin_coord is None) and prj.is_in_project(t):
                        if t.drawn_x_end_coord < self.drawn_x_begin_coord:
                            ...
                            marker.add(svgwrite.shapes.Circle((5, 5), r=5, fill=DEFAULT_DEP_LINE_COLOR, opacity=0.5, stroke_width=0))
                            svg.add(marker)
                            # vertical line
...
class Milestone(Task):
    ...
    def __init__(self, name, start=None, depends_of=None, color=None, fullname=None, display=True):
        ...
        if color is not None:
            ...
            self.color = DEFAULT_MILESTONE_COLOR
    ...
    def _svg_calendar(self, maxx, maxy, start_date, today=None, scale=DRAW_WITH_DAILY_SCALE, offset=0):
        ...
        for x in range(maxx):
            ...
            if scale == DRAW_WITH_DAILY_SCALE:
                ...
                # Month name
                if divmod(x, 4)[1] == 0:
                    vlines.add(svgwrite.text.Text('{0}'.format(str('M'+str(1 + divmod(x, 4)[0]))),
                                                  insert=((x*10+1+offset)* mm * _scale.custom_x_scale, 10*mm),
                                                  fill=DEFAULT_MONTH_COLOR, stroke=DEFAULT_MONTH_COLOR, stroke_width=0,
                                                  font_family=_font_attributes()['font_family'], font_size=15+3, font_weight="bold"))
                ...
                if divmod(x, 12)[1] == 0 and x !=0:
                    vlines.add(svgwrite.text.Text('Y'+str(1 + divmod(x, 12)[0]),
                                                  insert=((x*10+1+offset)* mm * _scale.custom_x_scale, 5*mm),
                                                  fill=DEFAULT_YEAR_COLOR, stroke=DEFAULT_YEAR_COLOR, stroke_width=0,
                                                  font_family=_font_attributes()['font_family'], font_size=15+5, font_weight="bold"))
                ...
                if (divmod(x, 4)[1]  == 0) and x != 0:
                    vlines.add(svgwrite.text.Text('Y'+str(1 + divmod(x, 4)[0]),
                                                  insert=((x*10+1+offset)* mm * _scale.custom_x_scale, 5*mm),
                                                  fill=DEFAULT_YEAR_COLOR, stroke=DEFAULT_YEAR_COLOR, stroke_width=0,
                                                  font_family=_font_attributes()['font_family'], font_size=15+5, font_weight="bold"))
